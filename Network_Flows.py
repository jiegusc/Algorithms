...
# had_past = [False] * 5
# dis[0] = 0
# while had_past.count(False):
#     min_dis = float('-inf')
#     min_ind = -1
#     for i in range(5):
#         if not had_past[i] and dis[i] > min_dis:
#             min_dis = dis[i]
#             min_ind = i
#     if min_ind == -1:
#         break
#     else:
#         had_past[min_ind] = True
#     for j in range(len(start_nodes)):
#         if(start_nodes[j] == min_ind):
#             dis[end_nodes[j]] = max(dis[end_nodes[j]], min_dis + capacities[j])
#
# print(dis)
################################################################################################
...
# 求 max(x + 2y + 3z)
# s.t. 2x + y + z <= 20, x + y + z <=15, x - y - z >= 0, x,y,z >= 0
